...
void linkup( int num_pipes, int num_nodes, bool &debug, matrix <int> &node, vector <int> &ipt, vector <int> &link)
{
//
...
//
	...
	{
		pos_node = node(branch_id, 1);
		neg_node = node(branch_id, 2) ;
	...
	if (header_output) // if there are any 0s in cout:
	{
		//print 'pretty' error
	...
	{
		pos_node = node(pipe,1);
    neg_node = node(pipe,2);
    --ipt(pos_node);
